...
match_array = [[0, 0, 60], [0, 60, 100], [60, 100, 100]]
...
edu_level_low = fuzz.interp_membership(edu.get_fuzzy_range(), edu.get_low_set(), 60)
edu_level_med = fuzz.interp_membership(edu.get_fuzzy_range(), edu.get_med_set(), 60)
edu_level_high = fuzz.interp_membership(edu.get_fuzzy_range(), edu.get_high_set(), 60)

age_level_low = fuzz.interp_membership(age.get_fuzzy_range(), age.get_low_set(), 50)
age_level_med = fuzz.interp_membership(age.get_fuzzy_range(), age.get_med_set(), 50)
age_level_high = fuzz.interp_membership(age.get_fuzzy_range(), age.get_high_set(), 50)

fin_level_low = fuzz.interp_membership(fin.get_fuzzy_range(), fin.get_low_set(), 20)
fin_level_med = fuzz.interp_membership(fin.get_fuzzy_range(), fin.get_med_set(), 20)
fin_level_high = fuzz.interp_membership(fin.get_fuzzy_range(), fin.get_high_set(), 20)

rule1 = np.fmax(edu_level_low, fin_level_low)

match_level_low = np.fmin(rule1, match.get_low_set())

match_level_mid = np.fmin(age_level_med, match.get_med_set())

rule3 = np.fmax(edu_level_high, fin_level_high)
